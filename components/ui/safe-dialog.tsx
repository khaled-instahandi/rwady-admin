...
const useBodyPointerEventsCleanup = (open: boolean) => {
  React.useEffect(() => {
    if (!open) {
      // Force cleanup of pointer-events when dialog closes
      const timer = setTimeout(() => {
        ...
        if (body.getAttribute('style') === 'pointer-events: none;' || 
            body.getAttribute('style') === 'pointer-events: none') {
  ...
  // Also force cleanup on component unmount
  React.useEffect(() => {
    return () => {
      const body = document.body
      if (body.style.pointerEvents === 'none') {
...
>(({ open, onOpenChange, ...props }, ref) => {
  useBodyPointerEventsCleanup(open || false)
